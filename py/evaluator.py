...
def run():
    queue = os.listdir(C.WEIGHTS_DIRECTORY + "/eval/queue")
    queue_int = []
    ...
    for w in queue_int:
        best = os.listdir(C.WEIGHTS_DIRECTORY + "/best")[0]
        ...
        result_dir = C.WEIGHTS_DIRECTORY + "/eval/result/{0}_{1}".format(w_no, best_no)
        ...
        for i in range(C.EVAL_PROCESS_COUNT):
            os.system(
                "start ../gomokuer.exe weval -w0 {wdir}/{w}.pkl -w1 {wdir}/{best}.pkl -r {r} -o {o}".
                format(
        ...
        while True:
            ...
            if len(results) >= C.EVAL_TARGET_ROUNDS:
                ...
                if ratio >= 1.2:
                    ...
                    os.rename(C.WEIGHTS_DIRECTORY + "/best/{0}".format(best_no),
                              C.WEIGHTS_DIRECTORY + "/best/{0}".format(w_no))
                    logstr += "PASS"
                else:
                    print("weight {w} failed.".format(w=w_no))
                    logstr += "FAIL"
                os.remove(C.WEIGHTS_DIRECTORY + "/eval/queue/{0}".format(w_no))

                with open(C.WEIGHTS_DIRECTORY + "/log/" + logstr, mode="w"):
                    pass
